...
// Every time NEWLINE is encountered, the preprocessor will replace it with
// printf("\n")
...
double part_dec(double number) {
  return number - (int)number; // To grab the decimal part I just do a cast to
                               // take the integer part and substract it
}
...
int sum_of_squares(int n) {
  if (n == 1) {
    ...
    return n * n +
           sum_of_squares(n -
                          1); // n * n is for the current n and then I call it
                              // on n-1 until n reach 1 which is the "base case"
  }
}

// If there is no void in parameters, it creates a warning when compiling.
/*This happens because the compiler assume that if there are no parameters it
could take any number of parameters, by providing void in parameters the
compiler understand that it takes no parameters.*/
...
int main(int argc, char *argv[]) {
  ...
  do {
    ...
    case 2: {
      // We need a separate scope here to avoid conflict with the 'sum' declared
      // in case 1
      printf("Executing Exercise 2\n");

      printf("\nSelect an int k to calculate the sum of the squares from 1 to "
             "k : ");
      int k;
      scanf("%d", &k);
      NEWLINE;
    ...
    case 3: {
      ...
      /*If the user enter a negative number in exercise 2 the output of the
      function is always 0 since k is < 0, we don't enter the for because i is
      equals to 1 and it is > than a negative number hence the result is the
      initialisation of sum wich is 0 */
      NEWLINE;
      int k;
      do {
        printf("Select an int k > 0 to calculate the sum of the squares from 1 "
               "to k : ");
        scanf("%d", &k);
      } while (k < 0);
      NEWLINE;
    ...
    case 9:
      ...
      titi(&z); // Here I'm passing the address of z in the function and titi
                // will modify the int it points to
      printf("After executing the fixed toto function titi, z is equal to %d "
             "in main\n",
             z);
